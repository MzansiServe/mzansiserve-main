...
import { TrendingUp, Wallet, CheckCircle, Car, Clock, ChevronRight, MapPin, Bell } from 'lucide-react-native';
...
export default function DriverDashboard() {
  ...
  const earnings = data?.driver_earnings || 0;
  const balance = data?.wallet?.balance || 0;
  const completed = data?.recent_rides?.length || 0;
  const available = data?.available_cab_requests?.length || 0;
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <View>
          <Typography variant="h2" weight="bold">Driver Console</Typography>
          <View style={styles.statusBadge}>
            <View style={styles.statusDot} />
            <Typography variant="caption" color={COLORS.secondary} weight="bold">ONLINE</Typography>
          </View>
        </View>
      ...
      <View style={styles.statsGrid}>
        ...
        <StatCard title="AVAILABLE" value={available} icon={Car} color={COLORS.accent} />
      </View>

      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          <Typography variant="h3" weight="bold">Recent Activity</Typography>
          <TouchableOpacity>
        ...
        {data?.recent_rides?.length > 0 ? (
          data.recent_rides.map((ride: any) => (
            <Card key={ride.id} shadow="sm" style={styles.activityItem}>
              <View style={styles.activityIcon}>
                <Clock size={20} color={COLORS.gray[400]} />
              </View>
              <View style={styles.activityInfo}>
                <Typography variant="subtitle" weight="bold">
                  {ride.pickup_address?.split(',')[0] || 'Local Ride'}
                </Typography>
                <View style={styles.activityMeta}>
                  ...
                  <Typography variant="caption" color={COLORS.gray[500]}>Completed</Typography>
                </View>
              </View>
          ...
          <View style={styles.emptyContainer}>
            <Clock size={48} color={COLORS.gray[200]} />
