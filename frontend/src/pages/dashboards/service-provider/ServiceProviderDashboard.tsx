...
const ServiceProviderDashboard = () => {
    ...
    const navItems = [
        ...
        { id: "active", label: "Active Bookings", icon: Clock, href: "#active", onClick: () => setActiveTab("active") },
        { id: "rides", label: "Available Requests", icon: ClipboardList, href: "#rides", onClick: () => setActiveTab("rides") },
            ...
            <div className="mb-8 rounded-2xl border border-amber-200 bg-amber-50 p-6">
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-amber-700 mb-4">Complete these steps to start receiving service bookings:</p>
                        <div className="space-y-2">
                            ...
                            <div className={cn("flex items-center gap-2 text-sm", user.id_verification_status === 'verified' ? "text-green-700" : "text-amber-700")}>
                                ...
                                {user.id_verification_status === 'verified' ? "ID documents verified ✓" : "Upload your business registration / ID via your profile"}
                            </div>
        ...
        switch (activeTab) {
                                ...
                                {(data?.active_service_provider_jobs?.length > 0) && (
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            <h2 className="text-lg font-bold text-[#121926]">Ongoing Booking</h2>
            ...
            case "reviews":
                return (
                    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden h-full animate-in fade-in duration-500">
                        ...
                        <div className="divide-y divide-gray-100">
                            {data?.recent_service_provider_jobs?.filter((r: any) => r.has_provider_rating).length > 0 ? (
                                data.recent_service_provider_jobs.filter((r: any) => r.has_provider_rating).map((job: any) => (
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-1">
                                                {[1, 2, 3, 4, 5].map((star) => (
                                                    <Star 
                                                        key={star} 
                                                        className={cn("h-4 w-4", star <= job.details?.provider_rating ? "text-amber-400 fill-current" : "text-slate-200")} 
                                                    />
                                                ))}
                                            </div>
                                            <span className="text-xs text-slate-400">{new Date(job.created_at).toLocaleDateString()}</span>
                                        </div>
                                        <p className="text-sm text-slate-600 italic">"{job.details?.provider_review || "No comments provided."}"</p>
                                        <p className="text-[10px] text-slate-400 mt-2 uppercase font-bold tracking-widest">Job: {job.details?.service_name}</p>
                                    </div>
                                ))
                            ) : (
                                <div className="p-12 text-center text-[#697586] italic text-sm">
                                    No reviews yet. Complete more bookings to receive feedback!
                                </div>
                            )}
                        </div>
                    </div>
            ...
            case "services":
                return (
                    <div className="animate-in fade-in duration-500">
                        <ServiceManagement
                            initialServices={data?.profile_data?.provider_services || []}
                            role="service-provider"
                        />
                    </div>
                );
    ...
    return (
        <ProviderDashboardLayout
            ...
            subtitle="Monitor your bookings, manage earnings, and provide top-tier services."
            navItems={navItems}
