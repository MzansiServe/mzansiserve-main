#!/usr/bin/env python3
"""Run docker compose down on remote host via SSH. Exit 0 on success."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run docker compose down on remote host.")
    ...
    cmd = "cd '%s' && docker compose down" % dir_esc
    if args.file:
        file_esc = args.file.replace("'", "'\"'\"'")
        cmd = "cd '%s' && docker compose -f '%s' down" % (dir_esc, file_esc)
