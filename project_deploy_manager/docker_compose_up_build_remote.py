#!/usr/bin/env python3
"""Run docker compose up -d --build on remote host via SSH. Exit 0 on success."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run docker compose up -d --build on remote host.")
    ...
    cmd = "cd '%s' && docker compose up -d --build" % dir_esc
    if args.file:
        file_esc = args.file.replace("'", "'\"'\"'")
        cmd = "cd '%s' && docker compose -f '%s' up -d --build" % (dir_esc, file_esc)
    ...
    try:
        ...
        return 1
    try:
        code, out, err = run_remote_stream(client, cmd, timeout_sec=600)
    finally:
        client.close()
    if code != 0:
