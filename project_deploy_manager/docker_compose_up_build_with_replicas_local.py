#!/usr/bin/env python3
"""Run docker compose up -d --build with scale/replicas locally. Service name and count as args. Exit 0 on success."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run docker compose up -d --build with scale (replicas) locally.")
    ...
    cmd_parts.extend(["up", "-d", "--build", "--scale", "%s=%d" % (shlex.quote(args.service), args.replicas)])
    code, out, err = run_local(" ".join(cmd_parts), timeout_sec=600)
    if code != 0:
